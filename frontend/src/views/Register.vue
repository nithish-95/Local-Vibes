<template>
  <div class="flex items-center justify-center min-h-screen bg-light-bg px-4 py-8">
    <div class="w-full max-w-md p-8 space-y-6 bg-white rounded-lg shadow-xl">
      ...
      <h2 class="text-3xl font-serif text-center text-light-text">Create a New Account</h2>
      <Form @submit="register" :validation-schema="schema" :validate-on-mount="false" class="space-y-6">
        ...
        <div>
          <button type="submit" class="align-middle select-none font-sans font-bold text-center uppercase transition-all disabled:opacity-50 disabled:shadow-none disabled:pointer-events-none text-xs py-3 px-6 rounded-lg bg-primary-500 text-dark-text shadow-md hover:shadow-lg focus:opacity-[0.85] focus:shadow-none active:opacity-[0.85] active:shadow-none w-full">Register</button>
        </div>
      </Form>
      <p class="text-sm text-center text-neutral-600 font-sans">
        Already have an account? 
        <router-link to="/login" class="font-medium text-primary-500 hover:underline">Login</router-link>
      </p>
    </div>
  </div>
</template>
